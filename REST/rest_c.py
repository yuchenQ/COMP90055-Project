...
# usage:
# Scraping via REST_API with geocode and query
# localhost:
...
from info import query_info, db_info
...
def main():
    ...
    print("-- Geo scope: {} ".format(args.query))
    print("Start Downloading max {} tweets...\n".format(maxTweets))

    while tweetCount < maxTweets:
        try:
            if max_id <= 0:
                new_tweets = app_api.search(q=query_info[args.query],
                                            lang=args.lang,
                                            count=tweetsPerQry)
            else:
                new_tweets = app_api.search(q=query_info[args.query],
